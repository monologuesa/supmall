...
// var imgSrc = "https://youte.xmtzxm.com.cn/weixinpl/youte/common_img/02.png"
...
Page({
  ...
  onLoad: function (options) {
    ...
    if (options.scene) {
      ...
      that.setData({
        image: getApp().openidData.urls,//域名
      ...
      that.setData({
        batchcode: options.batchcode,//祈福的订单号
        image: getApp().openidData.urls//域名
      })
  ...
  getImage_1:function(){
    var that = this;
    getApp().getImg();
    var imgSrc = that.data.image + that.data.index_data.imgurl
    console.log(imgSrc)
    wx.downloadFile({
      url: imgSrc,
  ...
  onShareAppMessage:function(res){
    ...
    return {
      title: '精选游玩购',
      imageUrl: that.data.index_data_1.url_path // 图片 URL
    }
  },
  //继续请香
  comment: function () {
  ...
  hide: function () {
    var that = this;
    that.setData({ hide: true })//点击显示小程序码
  },
  //隐藏
  hideModal: function () {
    // 隐藏遮罩层
