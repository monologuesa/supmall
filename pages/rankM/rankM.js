...
Page({
  ...
  onLoad: function (options) {
    ...
    that.setData({ check: true,name:'微信支付' })
    getdata(that);
  },
  //点击切换支付方式
  click:function(e){
   ...
   let data = that.data.index_data.refresh_money;
   let data_1 = that.data.index_data.refresh_integral;
   if(name == '微信支付'){
  ...
  pay:function(){
    ...
    if(that.data.name == '积分支付'){
      wx.request({
        url: getApp().openidData.url + 'save_refresh_order.php',
        data: {
        ...
        success: function (res) {
          ...
          if (res.data.status == 2){
            ...
            setTimeout(function () {
              wx.redirectTo({
                url: '/pages/refreshPay/refreshPay?sertial=3' + '&batchcode=' + that.data.batchcode,
              });
            }, 1000)
          }else{
            wx.showModal({
      ...
      wx.request({
        ...
        success: function (res) {
          ...
          wx.request({
            url: 'https://youte.xmtzxm.com.cn/weixinpl/mshop/WeChatPay/wxpay_refresh.php',
            data: {
            ...
            success: function (re) {
              ...
              wx.requestPayment({
                ...
                'success'(res) {
                  ...
                  setTimeout(function () {
                    wx.redirectTo({
                      url: '/pages/refreshPay/refreshPay?batchcode=' + that.data.batchcode + '&sertial=3',
                    });
                  }, 1000)
                },
                fail(res) {
...
function getdata(that) {
  wx.request({
    url: getApp().openidData.url + 'refresh_list.php',
    data: {},
    method: 'POST',
    header: {
      "Content-Type": "application/x-www-form-urlencoded"
    ...
    success: function (res) {
      ...
      var content = "<div style=\"line-height:25px; font-weight:200; font-size:17px; color:black; word-break:normal\">" + res.data.refresh_content + "</div>";
