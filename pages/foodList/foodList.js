Page({
  ...
  onLoad: function (options) {
    var that = this;
    that.setData({
      type: options.type,//获取点击切换的
      imageUrl: getApp().openidData.urls,//照片的前缀
      city_1:getApp().openidData.city//默认的城市
    })
    wx.getLocation({
      type: 'gcj02',
      success(res) {
  ...
  form: function (e) {
    var that = this;
    let name = e.detail.value.text;//搜索框中的值
  ...
  click: function (e) {
    var that = this;
    let id = e.currentTarget.dataset.id;//获取列表的id
    let index = e.currentTarget.dataset.index;//索引值
    that.setData({
      id:id,
      type: index + 1,
    })
    getlist(that.data.type_1,that.data.latitude, that.data.longitude, that.data.city, id, that)
  },
  //删除照片
  clearImg: function (e) {
